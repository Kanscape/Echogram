...
class AppShell extends StatelessComponent {
  ...
  Component build(BuildContext context) {
    ...
    return div(classes: 'relative min-h-screen text-slate-900 dark:text-slate-100', [
      div(classes: 'echo-backdrop pointer-events-none fixed inset-0 opacity-70', []),
      div(classes: 'relative mx-auto flex min-h-screen w-full max-w-[1500px] flex-col gap-6 px-4 py-6 lg:px-8', [
        div(classes: 'shell-glass rounded-[2rem] px-6 py-5', [
          div(classes: 'flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between', [
            div(classes: 'flex items-center gap-4', [
              div(classes: 'inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-slate-900 text-sm font-bold uppercase tracking-[0.24em] text-white shadow-lg dark:bg-white dark:text-slate-950', [
                .text('EG'),
              ]),
              div(classes: 'space-y-1', [
                div(classes: 'text-xs font-bold uppercase tracking-[0.24em] text-teal-700 dark:text-teal-300', [.text(t.brandLabel)]),
                p(classes: 'text-sm text-slate-600 dark:text-slate-300', [
            ...
            div(classes: 'flex flex-wrap items-center gap-2', [
              ...
              div(classes: 'rounded-full bg-teal-600 px-3 py-1 text-xs font-semibold text-white shadow-md', [
                .text(t.autoLanguageBadge),
              ]),
              div(classes: 'rounded-full bg-white/80 px-3 py-1 text-xs font-medium text-slate-600 shadow-md dark:bg-slate-900/85 dark:text-slate-300 dark:shadow-none', [
